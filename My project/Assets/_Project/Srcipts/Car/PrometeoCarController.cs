...
public class PrometeoCarController : NetworkBehaviour
{
    ...
    [Range(0, 10)] public int decelerationMultiplier = 2;
    ...
    // Свойства для NetworkCarController
    public float throttleAxis_Network => throttleAxis;
    public float steeringAxis_Network => steeringAxis;
    ...
    // Определяем: управляет ли этот экземпляр локальный игрок
    // В одиночной игре Mirror не активен — управление всегда разрешено
    private bool IsLocallyControlled =>
        !NetworkClient.active || isOwned;
    ...
    {
        // -------------------------------------------------------
        // Чужая машина в сетевой игре: только анимируем колёса
        // В одиночной игре (NetworkClient.active == false) — всегда управляем
        // -------------------------------------------------------
    ...
    public void HandBrake()
    {
        ...
        else
        {
            if (Mathf.Abs(Mathf.RoundToInt(carSpeed)) < maxReverseSpeed)
            {
                ...
                frontLeftCollider.motorTorque = (accelerationMultiplier * 200f) * throttleAxis;
                frontRightCollider.brakeTorque = 0;
                frontRightCollider.motorTorque = (accelerationMultiplier * 200f) * throttleAxis;
